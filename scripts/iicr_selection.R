...
filename=args[1] # file ID
N=as.integer(args[2]) # diploid population size
E=as.integer(args[3]) # number of time windows used to discretize the IICR 

u=read.table(paste(filename,'.times',sep=''),head=T)
nb_s=max(u$sample)
...
# plot
p=ggplot(res,aes(x=t,y=IICR,color=as.factor(sample)))+geom_line()+theme_bw()+scale_x_continuous(limits=c(1000,100000),trans='log10')+scale_color_discrete(name = "region")+ylim(0,2)+geom_line(aes(x=t,y=IICR_av),color='black')+xlab('generations')
ggsave(paste(filename,'_E',E,'.jpg',sep=''),plot = p, width = 5, height = 4)
